...
import ScoreSummaryTable from "../src/components/ScoreSummaryTable.vue";
...
describe("ScoreBoard page:", () => {
    ...
    it("data reflects store", () => {
        const wrapper = mount(ScoreSummaryTable, {
            localVue,
            store,
        });

        const teams = wrapper.vm.teams;
        const questions = wrapper.vm.questions;
        const themes = wrapper.vm.themes;
        const flow = wrapper.vm.flow;
        const themesForRender = wrapper.vm.forScoreRender;

        expect(teams).toBe(store.getters.teamsSortedByScore);
        expect(questions).toBe(store.state.quiz.questions);
        expect(themes).toBe(store.state.quiz.themes);
        expect(flow).toBe(store.state.quiz.flow);
        expect(themesForRender).toBe(store.getters.themesQWithDummy);
    });
});
